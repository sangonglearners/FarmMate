...
/** 🔧 한글/영문 뒤섞인 입력 → 영문 키로 통일 */ export function normalizeRow(row) {
  return {
...
async function runTests() {
  ...
  for(let i = 0; i < testCases.length; i++){
    ...
    try {
      const result = await engine.recommendCrops(testCase.startMonth, testCase.endMonth, testCase.inputPlace, testCase.inputIrang, sampleCropsData);
      if (result.error) {
