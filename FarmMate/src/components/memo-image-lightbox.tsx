  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="max-w-[min(100vw-1rem,56rem)] w-full p-0 gap-0 overflow-hidden border-0 bg-black/95 text-white sm:rounded-lg z-[200]"
      >
        <DialogHeader className="sr-only">
          <DialogTitle>{barTitle}</DialogTitle>
        </DialogHeader>
        <div className="relative flex flex-col max-h-[90vh]">
          ...
          <div className="relative flex items-center justify-center min-h-[200px] bg-black">
            {current?.url ? (
              <img
                src={current.url}
                alt=""
          ...
          {(current?.caption || safeSlides.length > 1) && (
            <div className="px-3 py-2 border-t border-white/10 text-xs text-white/80 space-y-1">
              {safeSlides.length > 1 && (
                <p>
