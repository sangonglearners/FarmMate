...
import { format, startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfYear, endOfYear, startOfQuarter, endOfQuarter, eachDayOfInterval, eachWeekOfInterval, eachMonthOfInterval, eachQuarterOfInterval, eachYearOfInterval, subWeeks, subMonths, subYears } from "date-fns";
...
const generateRevenueDataFromLedgers = (
  ...
  switch (periodType) {
    ...
    case "quarterly": {
      // 가장 최신 분기 기준 최근 4개 분기
      const start = subYears(today, 1);
      const quarters = eachQuarterOfInterval({ start, end: today });
      const lastFour = quarters.slice(-4);
      
      return lastFour.map((quarterStart, index) => {
        ...
        const revenue = ledgers
          .filter((ledger) => {
            ...
            if (!task) return false;
            const taskEndDate = (task as any).endDate || task.scheduledDate;
            return taskEndDate >= quarterStartStr && taskEndDate <= quarterEndStr;
          })
          .reduce((sum, ledger) => sum + (ledger.revenueAmount || 0), 0);
...
export default function StatsPage() {
  ...
  const revenueData = useMemo(() => {
    return generateRevenueDataFromLedgers(allLedgers, tasks, periodType);
  }, [allLedgers, tasks, periodType]);
