...
### Phase 1: UI 구성 (2시간) ✅ 완료
- [x] `farm-calendar-grid.tsx`에 내보내기 버튼 추가
- [x] CSV Export 기능 구현 (다이얼로그 없이 직접 다운로드)
- [x] 버튼 상태 관리 (useState)

### Phase 2: CSV 생성 로직 (2시간) ✅ 완료
- [x] CSV 생성 로직 구현
- [x] CSV 컬럼 헤더 정의
- [x] 데이터 필터링 함수 구현
  - [x] 날짜 범위 필터링 (scheduledDate, endDate 고려)
  - [x] 선택된 농장의 작업만 포함
- [x] 작업-작물 데이터 조인
- [x] CSV 문자열 생성 (쉼표, 따옴표 이스케이프 처리)
- [x] UTF-8 BOM 추가 (Excel 호환성)

### Phase 3: 다운로드 기능 (1시간) ✅ 완료
- [x] 파일 다운로드 함수 구현
- [x] 파일명 생성 로직 (`farmmate_calendar_YYYY-MM-DD.csv`)
- [x] 버튼 클릭 시 즉시 다운로드

### Phase 4: 테스트 및 버그 수정 (1시간) ✅ 완료
- [x] 월간 뷰에서 내보내기 테스트
- [x] 연간 뷰에서 내보내기 테스트
- [x] 한글 파일명 테스트 (Windows/Mac)
- [x] CSV 파일을 Excel에서 열어 한글 깨짐 확인
- [x] 빈 데이터 처리 (작업 없는 경우)
...
### 주요 기능
- ✅ 캘린더 페이지에 CSV Export 버튼 추가
- ✅ 현재 표시된 작업 데이터를 CSV 파일로 변환
- ✅ UTF-8 BOM 인코딩으로 한글 호환성 보장
- ✅ 파일명에 현재 날짜 포함

### 구현 파일
- `FarmMate/src/components/farm-calendar-grid.tsx` (113줄 추가)
- `FarmMate/src/pages/calendar/ui/CalendarPage.tsx` (56줄 추가)
